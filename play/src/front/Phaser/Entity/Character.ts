...
export abstract class Character extends Container implements OutlineableInterface {
    ...
    protected playAnimation(direction: PositionMessage_Direction, moving: boolean): void {
        ...
        // If sitting, force sit animation and ignore movement
        if (this._isSitting) {
            for (const [texture, sprite] of this.sprites.entries()) {
                if (!sprite.anims) return;
                const sitKey = texture + "-down-" + PlayerAnimationTypes.Sit;
                if (!sprite.anims.currentAnim || sprite.anims.currentAnim.key !== sitKey) {
                    sprite.anims.play(sitKey, true);
                }
            }
            return;
        }
    ...
    public setSitting(sitting: boolean): void {
        ...
        if (sitting) {
            // Offset sprites down to simulate sitting on a chair
            for (const sprite of this.sprites.values()) {
                sprite.setY(sprite.y + 4);
            }
            // Force sit animation facing down
