...
export const localeDetector = async () => {
    if (!initPromise) {
        initPromise = (async () => {
            ...
            if (exist) {
                // User explicitly chose a language — respect their preference
            ...
            } else if (FALLBACK_LOCALE && isLocale(FALLBACK_LOCALE)) {
                // Admin has configured a forced locale via FALLBACK_LOCALE — use it for all new users
                detectedLocale = FALLBACK_LOCALE as Locales;
            } else {
